...
const CRYONIS_BUILDING_ROOMS = ['cryo_arcade', 'cryo_arcade_top', 'cryo_plaza', 'cryo_club', 'cryo_club_roof', 'cryo_familymart', 'cryo_seven'];
...
io.on('connection', (socket) => {
  ...
  socket.on('tetris-score', async (d) => {
    ...
    socket.emit('stardust', { value: me.stardust });
    if (xp > 0) grantXp(socket, me, xp);
    const isRecord = score > (me.tetrisBest || 0);
    if (isRecord) me.tetrisBest = score;
    socket.emit('tetris-best', { best: me.tetrisBest, record: isRecord });
    admin.from('profiles').update({ stardust: me.stardust, xp: me.xp, tetris_best: me.tetrisBest }).eq('id', me.userId).then(() => {}, () => {});
  });

  socket.on('tetris-leaderboard', async () => {
    const { data } = await admin.from('profiles')
      .select('username,tetris_best')
      .order('tetris_best', { ascending: false })
      .limit(10);
    socket.emit('tetris-leaderboard-data', { rows: (data || []).filter(r => (r.tetris_best || 0) > 0) });
  });

  socket.on('pacman-score', async (d) => {
    const me = players[socket.id]; if (!me) return;
    const now = Date.now();
    if (now - (me.lastPacman || 0) < 3000) return;
    me.lastPacman = now;
    const score = Math.max(0, Math.min(999999, Math.round(+d.score || 0)));
    const gained = Math.min(500, Math.round(score / 6));
    const xp = Math.min(150, Math.round(score / 20));
    me.stardust += gained;
    socket.emit('stardust', { value: me.stardust });
    if (xp > 0) grantXp(socket, me, xp);
    const isRecord = score > (me.pacmanBest || 0);
    if (isRecord) me.pacmanBest = score;
    socket.emit('pacman-best', { best: me.pacmanBest, record: isRecord });
    admin.from('profiles').update({ stardust: me.stardust, xp: me.xp, pacman_best: me.pacmanBest }).eq('id', me.userId).then(() => {}, () => {});
  });

  socket.on('pacman-leaderboard', async () => {
    const { data } = await admin.from('profiles')
      .select('username,pacman_best')
      .order('pacman_best', { ascending: false })
      .limit(10);
  ...
  socket.on('stepbeat-score', async (d) => {
    const me = players[socket.id]; if (!me) return;
    const now = Date.now();
    if (now - (me.lastStepbeat || 0) < 3000) return;
    me.lastStepbeat = now;
    const score = Math.max(0, Math.min(999999, Math.round(+d.score || 0)));
    ...
    socket.emit('stardust', { value: me.stardust });
    if (xp > 0) grantXp(socket, me, xp);
    const isRecord = score > (me.stepbeatBest || 0);
    if (isRecord) me.stepbeatBest = score;
    socket.emit('stepbeat-best', { best: me.stepbeatBest, record: isRecord });
    admin.from('profiles').update({ stardust: me.stardust, xp: me.xp, stepbeat_best: me.stepbeatBest }).eq('id', me.userId).then(() => {}, () => {});
  });

  socket.on('stepbeat-leaderboard', async () => {
    const { data } = await admin.from('profiles')
      .select('username,stepbeat_best')
      .order('stepbeat_best', { ascending: false })
      .limit(10);
    socket.emit('stepbeat-leaderboard-data', { rows: (data || []).filter(r => (r.stepbeat_best || 0) > 0) });
  });

  socket.on('bonkabot-score', async (d) => {
    const me = players[socket.id]; if (!me) return;
    const now = Date.now();
    if (now - (me.lastBonkabot || 0) < 3000) return;
    me.lastBonkabot = now;
    const score = Math.max(0, Math.min(999999, Math.round(+d.score || 0)));
    const gained = Math.min(500, Math.round(score / 10));
    const xp = Math.min(150, Math.round(score / 15));
    ...
    socket.emit('stardust', { value: me.stardust });
    if (xp > 0) grantXp(socket, me, xp);
    const isRecord = score > (me.bonkabotBest || 0);
    if (isRecord) me.bonkabotBest = score;
    socket.emit('bonkabot-best', { best: me.bonkabotBest, record: isRecord });
    admin.from('profiles').update({ stardust: me.stardust, xp: me.xp, bonkabot_best: me.bonkabotBest }).eq('id', me.userId).then(() => {}, () => {});
  });

  socket.on('bonkabot-leaderboard', async () => {
    const { data } = await admin.from('profiles')
      .select('username,bonkabot_best')
      .order('bonkabot_best', { ascending: false })
      .limit(10);
